...
int AnalysisWorkspaceSR4_2017TF()
{
	...
	vector<double> lumiscalefactors = { 59.09, 59.03 };	//SR4
	vector<string> srmasses = { "600", "700" };	//SR4

	TString Tsrmasses[2] = { "600", "700" };	//SR4
	...
	RooRealVar mbb("mbb", "m_{12}", 350, 800);	//SR4
	RooArgList vars(mbb);
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		TFile *f_signal_in_unbinned = new TFile(dir + "/input_doubleCB_SL/signal_m" + Tsrmasses[mass] + "_SR4.root", "READ");
		...
		//Output file
		TFile *fOut = new TFile("input_2017TF_SL/signal_workspace_" + Tsrmasses[mass] + ".root", "RECREATE");
